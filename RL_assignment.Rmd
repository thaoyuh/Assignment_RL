...
# Initialize an empty dataframe to store test statistics
test_statistics_df <- data.frame()
...
for (i in 2:ncol(result)) {
  ...
  test_statistics_df <- rbind(test_statistics_df, data.frame(Item = colnames(result)[i], Test_Statistic = test_result@teststat))
}

# Print the dataframe containing test statistics
print(test_statistics_df)  # stationary if the value is lower than the threshold -1.95 (0.05 significance level)
```
