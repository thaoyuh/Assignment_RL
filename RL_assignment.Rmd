...
click_count <- dfZozo80 %>%
  group_by(date) %>%
  summarise(click_count = n())

# Create the plot
ggplot(data = click_count, aes(x = date, y = click_count)) +
  geom_line() +
  labs(x = "Date", y = "Click Count", title = "Click Count by Date")
...
# Plot
ggplot(data = clicks_by_hour, aes(x = hour, y = click_count)) +
  geom_line() +
  labs(x = "Hour of the Day", y = "Click Count", title = "Click Count by Hour")

# Group by hour and date
clicks_hour_day <- dfZozo80 %>%
  group_by(hour,date) %>%
  summarise(click_count = sum(click))

# Plot
ggplot(data = clicks_hour_day, aes(x = hour+date, y = click_count)) +
  geom_line() +
  labs(x = "Time", y = "Click Count", title = "Click Count by Hour")
```
